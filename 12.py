...
mnist = datasets.MNIST(
    root='./image', train=True, transform=img_transform, download=True)
...
class discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 32, 5, padding=2),  # batch, 16, 28-5+1, 28
            nn.LeakyReLU(0.2, True),
            nn.AvgPool2d(2, stride=2),  # batch, 32, 12, 12
            )
        self.conv2 = nn.Sequential(
            nn.Conv2d(32, 64, 5, padding=2),  # batch, 64, 14-5+1, 14
            nn.LeakyReLU(0.2, True),
            nn.AvgPool2d(2, stride=2)  # batch, 32, 5, 5
        )
...
for epoch in range(num_epoch):
    ...
    if epoch == 0:
        ...
        save_image(real_images, './image/real_images.png')
 
    fake_images = to_img(fake_img.cpu().data)
    save_image(fake_images, './image/fake_images-{}.png'.format(epoch+1))
